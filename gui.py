...
class DropArea(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.label.setStyleSheet("font-size: 16px; color: #445; padding: 20px;")
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        title = QLabel("极简视频剪辑工具")
        title.setAlignment(Qt.AlignCenter)
        title.setStyleSheet("font-size: 24px; font-weight: bold; color: #223;")
        layout.addWidget(title)

        subtitle = QLabel("拖入视频，设置要剪掉的开头秒数，然后直接导出。")
        subtitle.setAlignment(Qt.AlignCenter)
        subtitle.setStyleSheet("font-size: 13px; color: #667;")
        ...
        self.ffmpeg_label = QLabel("FFmpeg: 检测中...")
        self.ffmpeg_label.setWordWrap(True)
        self.ffmpeg_label.setStyleSheet("color: #556; font-size: 12px;")
        ...
        self.detect_button = QPushButton("重新检测 FFmpeg")
    ...
    def check_ffmpeg(self):
        ...
        if self.ffmpeg_path:
            ffmpeg_version = check_ffmpeg_version(self.ffmpeg_path) or "版本信息读取失败"

            if self.ffprobe_path:
                ffprobe_version = check_ffprobe_version(self.ffprobe_path) or "版本信息读取失败"
                ...
                self.status_label.setText("FFmpeg 和 FFprobe 已就绪")
            else:
                self.ffmpeg_label.setText(
                    "FFmpeg: 已找到\n"
                    f"{self.ffmpeg_path}\n"
                    f"{ffmpeg_version}\n\n"
                    "FFprobe: 未找到\n"
                    "将尝试使用 FFmpeg 兼容模式读取视频时长。"
                )
                self.status_label.setText("FFmpeg 已就绪，但 FFprobe 不可用")
        else:
            self.ffprobe_path = None
            self.ffmpeg_label.setText(
                "FFmpeg: 未找到\n"
                "请将 ffmpeg.exe 和 ffprobe.exe 放在软件同目录，或加入系统 PATH。"
            )
    ...
    def set_processing_state(self, processing):
        ...
        self.detect_button.setEnabled(not processing)
